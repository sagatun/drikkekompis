...
export async function updateProductRatingsInFirestore() {
  try {
    console.log(
      `Starter oppdatering av produktvurderinger for butikkID: ${STORE_ID}`
    );
    const firestore = await getFirestoreInstance();
    const storeRef = firestore.collection("stores_new").doc(STORE_ID);
    ...
    for (const doc of snapshot.docs) {
      ...
      // produktID er verdien til nøkkelen
